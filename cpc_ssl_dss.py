...
class ModelBase(ABC):
    ...
    @abstractmethod
    def load_training_configuration(self, config, data_gen, val_gen, data_type):
        """
    ...
    @abstractmethod
    def load_prediction_configuration(self, model, config, test_gen, data_type):
        """
...
class Block(Model):
    """
    Super class for contrastive loss.
    """
...
class ContrastiveLoss(Block):
    """
    It creates the block that calculates the contrastive loss for given latent and context representations.
    ...
    def __init__(self, context_units, neg, steps, name = 'Contrastive_Loss'):
        """
        :param context_units: number of units of the context representations.
    ...
    def call(self, inputs, **kwargs):
        """
        :param inputs: a list with two elements, the latent and the context representations.
        :return: the calculated contrastive loss.
        """
        
        true_latent, context_latent = inputs

        # Linear transformation of latent representations into the vector space of context representations
...
class CPCModel(ModelBase):
    ...
    def load_training_configuration(self, config, data_gen, val_gen, data_type):
        """
        It instantiates the cpc model using the configuration parameters.
        :param config: a dictionary with the configuration for training.
        :param data_gen: a dictionary with the training set features.
        :param val_gen: a dictionary with the validation set features.
        ...
        super(CPCModel, self).load_training_configuration(config, data_gen, val_gen, data_type)

        # Model architecture of contrastive predictive coding: TCN_Intermediate -> Dropout -> GRU -> Dropout
        
        # Decreasing the resolution of raw song data at the top layer for contrastive predictive coding model
        if self.data_type == 'flies_single_channel' or self.data_type == 'flies_multi_channels':
            max_pooling_size_cpc = 100
        
        elif self.data_type == 'birds_single_channel':
            max_pooling_size_cpc = 16
        ...
        if self.model_name == 'tcn_seq' or self.model_name == 'tcn':
            ...
            if self.nb_pre_conv > 0:
                out = tcn_layer.TCN(nb_filters = self.nb_filters, kernel_size = self.kernel_size, nb_stacks = self.nb_conv, dilations = self.dilations,
                                    activation = self.activation, use_skip_connections = self.use_skip_connections, padding = self.padding,
                                    dropout_rate = self.dropout_rate, return_sequences = self.return_sequences,
                                    use_separable = self.use_separable, name = 'frontend')(out)
        ...
        if self.model_name == 'tcn_multi':            
            ...
            # x = AveragePooling1D(pool_size = max_pooling_size_cpc, strides = max_pooling_size_cpc)(x) 
            x = MaxPooling1D(pool_size = max_pooling_size_cpc, strides = max_pooling_size_cpc)(x)
        ...
        # pooling_features = AveragePooling1D(pool_size = max_pooling_size_cpc, strides = max_pooling_size_cpc)(intermediate_features) 
        pooling_features = MaxPooling1D(pool_size = max_pooling_size_cpc, strides = max_pooling_size_cpc)(intermediate_features)
    ...
    def load_prediction_configuration(self, model, config, test_gen, data_type):
        ...
        super(CPCModel, self).load_prediction_configuration(model, config, test_gen, data_type)
    ...
    def train(self, config):
        ...
        :return: the trained model is saved on disk.
        """
    ...
    def predict(self, config):        
        """
        Predicts a self-supervised contrastive predictive coding (CPC) model.
        :return: the results are saved on disk.
        """
